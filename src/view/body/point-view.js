...
function createPointOffersListTemplate(offersData, offersIds) {
...
function createPointTemplate(pointData, offerData) {
  const {
    base_price: price,
    date_from: dateFrom,
    date_to: dateTo,
  ...
  const offersListTemplate = createPointOffersListTemplate(offerData, offers);

  const favoriteClass = `event__favorite-btn ${isFavorite ? 'event__favorite-btn--active' : ''}`;
...
export default class CreatePoint extends AbstractView {
  ...
  get template() {
    return createPointTemplate(this.points, this.offers);
  }
}
